...
async function getCart(fetchDom) {
    ...
    // donc :
    if (cart !== null) {
        let test = cart.sort((a, b) => {  // sort pour trier mes id numériquement
...
function cartContainer(pCartContent, pFetchContent) {
    ...
    const quantityWarn = document.querySelector(".itemQuantity");
    quantityWarn.addEventListener('change', (ev) => {
      if (quantityWarn.value < 1 || quantityWarn.value > 100) {
